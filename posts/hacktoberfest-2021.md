...
For example, `Tridiagonal` matrices are implemented just as 3 vectors. one of length `n` and the others of length `n-1`, describing the main and off diagonals. All other elements in the matrix are zero, so they need not be stored. When adding a `Tridiagonal` matrix to another matrix, only these `3n-2` elements need to be touched, so the sum can be done faster than if nothing was known about the operands.

My first [really big foray](https://github.com/JuliaLang/julia/pull/28883) into this space was to optimize addition and multiplication when both operands were structured. Many of these methods were too conservative in their return types, returning dense or `SparseMatrixCSC` (the generic sparse matrix format), when a more appropriate return type was available. [I made a nice table](https://github.com/JuliaLang/julia/pull/28883#issue-353888643) describing all of the expected and actual return types. There were a few substantial speedups. For example, multiplying a lower and upper bidiagonal matrix was returning a dense array when a `Tridiagonal` was always sufficient. Specializing this method and using the correct output type resulted in a ~99% decrease in runtime.


I am happy to see others working in this area, especially in the (in my opinion) much tricker case of division. [One](https://github.com/JuliaLang/julia/pull/42343) related PR was even submitted as I wrote this article.

### This Year’s Finds

I found a few small bugs this year. I mostly find these just by playing with randomly constructed structured matrix types and seeing when things are kind of slow or don’t match the output when everything is converted to dense representations. Basically, I am just fuzzing the `LinearAlgebra` standard library by hand.
...
Anyway, this leads to some unusual bugs when there is an extra element, as it often gets acted upon by functions that are broadcasted over the off-diagonal:
...
The extra element can also cause an error to be thrown:
...
These were resolved by replacing instances where broadcasts over `S.ev` were being done with a broadcast over a view of just the first `n-1` elements.

A related [issue](https://github.com/JuliaLang/julia/issues/42477), regarding `triu!`, `tril!`, and `Tridiagonal(::SymTridiagonal)`, was found during this, but has not yet been resolved.

##### [[2]](https://github.com/JuliaLang/julia/pull/42574) `SparseMatrixCSC` crash when passed empty `Tridiagonal` or `SymTridiagonal` matrices.
...
When testing these types out, I like to just use the dense matrix constructors like `Bidiagonal(::Matrix)`, where I can pass in a matrix, and the structured matrix type automatically extracts only the relevant elements. It makes testing quicker, as I don’t need to make the correct sized vectors all the time.
...
It turns out that this works for types with `uplo` in some cases, but not others:
...
I had a good time again this year with Hacktoberfest and Julia. I got some good advice and helpful reviews on Github and the Julia Slack.